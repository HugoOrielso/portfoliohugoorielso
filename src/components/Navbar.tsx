...
const Navbar = () => {
  ...
  return (
    <header className="flex sm:p-1 flex-col top-0 left-0 items-center z-50 bg-[#24242428] backdrop-blur-md justify-center w-full mb-4 fixed sm:dark:border-b">
        <div className='flex w-full items-center justify-end p-2 dark:text-white border-b border-zinc-900 dark:border-white sm:hidden'>
        ...
        <nav className={`hidden sm:flex gap-2 p-1 w-full list-none items-center justify-center sm:max-w-[1000px] dark:text-white text-xs sm:text-base`}>
            {
                links.map(item=>{
                    return(
                        <li key={crypto.randomUUID()}>
                            <a href={`#${item.section}`} onClick={(e) => scrollToSection(e, `${item.section}`)} className="dark:text-zinc-300 transition-all duration-300 dark:hover:text-white p-1 rounded hover:bg-zinc-600">{item.name}</a>
        ...
        <nav className={`${open ? 'flex': 'hidden'} sm:hidden h-screen flex-col w-full list-none items-start gap-2 justify-start sm:max-w-[1000px] dark:text-white text-xs sm:text-base duration-300 mt-2`}>
            {
                links.map(item=>{
                    return(
                        <li key={crypto.randomUUID()} className='flex w-full'>
